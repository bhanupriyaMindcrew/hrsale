...
class GDHandler extends BaseHandler
{
	...
	protected function process(string $action)
	{
		...
		if(empty($dest)){
			$copy(0, $src, 0, 0, $this->xAxis, $this->yAxis, $this->width, $this->height, $origWidth, $origHeight);
		}elseif(empty($src)){
			$copy($dest, 0, 0, 0, $this->xAxis, $this->yAxis, $this->width, $this->height, $origWidth, $origHeight);
		}elseif(empty($this->xAxis)){
			if(empty($this->yAxis)){
				$copy($dest, $src, 0, 0, 0, 0, $this->width, $this->height, $origWidth, $origHeight);
			}else{
				$copy($dest, $src, 0, 0, 0, $this->yAxis, $this->width, $this->height, $origWidth, $origHeight);
			}
		}elseif(empty($this->width)){
			$copy($dest, $src, 0, 0, $this->xAxis, $this->yAxis, 0, $this->height, $origWidth, $origHeight);
		}elseif(empty($this->height)){
			$copy($dest, $src, 0, 0, $this->xAxis, $this->yAxis, $this->width, 0, $origWidth, $origHeight);
		}elseif(empty($origWidth)){
			$copy($dest, $src, 0, 0, $this->xAxis, $this->yAxis, $this->width, $this->height, 0, $origHeight);
		}elseif(empty($origHeight)){
			$copy($dest, $src, 0, 0, $this->xAxis, $this->yAxis, $this->width, $this->height, $origWidth, 0);
		}else{
