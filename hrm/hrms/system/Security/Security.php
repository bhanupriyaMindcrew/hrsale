...
class Security implements SecurityInterface
{
	...
	public function verify(RequestInterface $request)
	{
		...
		// 	if (! empty($request->getBody()) && ! empty(json_decode($request->getBody())) && json_last_error() === JSON_ERROR_NONE)
		// 	{
		...
		if ($request->hasHeader($this->headerName) && !empty($request->getHeader($this->headerName)->getValue())) {
			...
			if (!empty($requestBody)) {
				...
				if (json_last_error() !== JSON_ERROR_NONE) {
					$json = null; // Set json to null if there's a JSON error
				}
			}
			
			if (!empty($json) && isset($json->{$this->tokenName})) {
				$tokenName = $json->{$this->tokenName};
			} else {
				$tokenName = null;
			}
		}
		...
		// Does the token exist in both the POST/POSTed JSON and COOKIE arrays and match?
		if (!isset($token, $_COOKIE[$this->cookieName]) || !hash_equals($token, $_COOKIE[$this->cookieName])) {
			throw SecurityException::forDisallowedAction();
		}
