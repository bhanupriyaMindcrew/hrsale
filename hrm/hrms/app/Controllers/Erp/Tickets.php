	...
	public function add_ticket() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$ticket_priority = $this->request->getPost('ticket_priority',FILTER_DEFAULT);
				...
				if($user_info['user_type'] == 'staff'){
					...
					if($this->request->getPost('department_id',FILTER_DEFAULT) === ''){
						$Return['error'] = lang('Employees.xin_employee_error_department');
					} else if($this->request->getPost('employee_id',FILTER_DEFAULT) === ''){
	...
	public function add_ticket_reply() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$token2 = udecode($this->request->getPost('token2',FILTER_DEFAULT));	
				$data = [
					'company_id' => $company_id,
					'ticket_id' => $id,
					'sent_by'  => $usession['sup_user_id'],
