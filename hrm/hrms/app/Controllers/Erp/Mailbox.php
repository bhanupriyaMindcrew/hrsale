...
class Mailbox extends BaseController {
	...
	public function send_mail() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$staff_id = $this->request->getPost('staff_id',FILTER_DEFAULT);	
	...
	public function reply_mail() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$description = $this->request->getPost('description',FILTER_DEFAULT);	
				$mail_id = udecode($this->request->getPost('token',FILTER_DEFAULT));
				$mail_to = udecode($this->request->getPost('token2',FILTER_DEFAULT));
	...
	public function update_starmail_record() {
		...
		if($this->request->getVar('field_id')) {
			...
			$id = udecode($request->getVar('field_id',FILTER_DEFAULT));
			$star_val = $request->getVar('star_val',FILTER_DEFAULT);
			$Return['csrf_hash'] = csrf_hash();
	...
	public function update_important_mail_record() {
		...
		if($this->request->getVar('field_id')) {
			...
			//$id = udecode($this->request->getGet('field_id'));
			$id = udecode($request->getVar('field_id',FILTER_DEFAULT));
			$imp_val = $request->getVar('imp_val',FILTER_DEFAULT);
			$Return['csrf_hash'] = csrf_hash();
