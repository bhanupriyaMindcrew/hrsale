	...
	public function create_new_estimate() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$invoice_number = $this->request->getPost('estimate_number',FILTER_DEFAULT);
				$project_id = $this->request->getPost('project',FILTER_DEFAULT);
				$invoice_date = $this->request->getPost('estimate_date',FILTER_DEFAULT);
				$invoice_due_date = $this->request->getPost('estimate_due_date',FILTER_DEFAULT);
				$j=0;
				...
				$invoice_note = $this->request->getPost('estimate_note',FILTER_DEFAULT);
	...
	public function update_estimate() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$invoice_due_date = $this->request->getPost('estimate_due_date',FILTER_DEFAULT);
				$j=0;
				foreach($this->request->getPost('item',FILTER_DEFAULT) as $eitem_id=>$key_val){
