	...
	public function add_official_document() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$license_name = $this->request->getPost('license_name',FILTER_DEFAULT);
				$document_type = $this->request->getPost('document_type',FILTER_DEFAULT);
				$expiry_date = $this->request->getPost('expiry_date',FILTER_DEFAULT);
				$license_number = $this->request->getPost('license_number',FILTER_DEFAULT);
