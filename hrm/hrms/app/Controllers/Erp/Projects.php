...
class Projects extends BaseController {
	...
	public function timelogs_list() {
		...
		$segment_id = $this->request->getVar('project_val',FILTER_DEFAULT);
		$ifield_id = udecode($segment_id);
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
	...
	public function add_project() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$title = $this->request->getPost('title',FILTER_DEFAULT);
				$start_date = $this->request->getPost('start_date',FILTER_DEFAULT);
				$end_date = $this->request->getPost('end_date',FILTER_DEFAULT);
				$client_id = $this->request->getPost('client_id',FILTER_DEFAULT);
				$summary = $this->request->getPost('summary',FILTER_DEFAULT);
				$description = $this->request->getPost('description',FILTER_DEFAULT);
				$priority = $this->request->getPost('priority',FILTER_DEFAULT);
				$budget_hours = $this->request->getPost('budget_hours',FILTER_DEFAULT);
				$assigned_ids = implode(',',$this->request->getPost('assigned_to',FILTER_DEFAULT));
	...
	public function update_project() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$budget_hours = $this->request->getPost('budget_hours',FILTER_DEFAULT);
				$assigned_ids = implode(',',$this->request->getPost('assigned_to',FILTER_DEFAULT));
				$associated_goals = implode(',',$this->request->getPost('associated_goals',FILTER_DEFAULT));
				$employee_ids = $assigned_ids;
	...
	public function update_project_progress() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$progres_val = $this->request->getPost('progres_val',FILTER_DEFAULT);
	...
	public function add_timelogs() {
		...
		if ($this->request->getPost('type') === 'add_timelogs') {
			...
			if(!$this->validate($rules)){
				...
				$start_time = $this->request->getPost('start_time',FILTER_DEFAULT);
				$end_time = $this->request->getPost('end_time',FILTER_DEFAULT);
				$start_date = $this->request->getPost('start_date',FILTER_DEFAULT);
				$end_date = $this->request->getPost('end_date',FILTER_DEFAULT);
				$memo = $this->request->getPost('memo',FILTER_DEFAULT);
				$id = udecode($this->request->getPost('token',FILTER_DEFAULT));
				// total hours
	...
	public function add_attachment() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$file_title = $this->request->getPost('file_name',FILTER_DEFAULT);
				$id = udecode($this->request->getPost('token',FILTER_DEFAULT));
				$UsersModel = new UsersModel();
				$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
				if($user_info['user_type'] == 'staff'){
	...
	public function update_project_status() {
		...
		if($this->request->getVar('xfieldid')) {
			...
			$request = \Config\Services::request();
			$usession = $session->get('sup_username');
			$id = $this->request->getVar('xfieldid',FILTER_DEFAULT);
			$status = $this->request->getVar('xfieldst',FILTER_DEFAULT);
			$Return['csrf_hash'] = csrf_hash();
			$UsersModel = new UsersModel();
			$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
			if($user_info['user_type'] == 'staff'){
	...
	public function delete_project_note() {
		...
		if($this->request->getVar('field_id')) {
			...
			$request = \Config\Services::request();
			$usession = $session->get('sup_username');
			$id = $this->request->getVar('field_id',FILTER_DEFAULT);
			$Return['csrf_hash'] = csrf_hash();
			$ProjectnotesModel = new ProjectnotesModel();
