...
class Clients extends BaseController {
	...
	public function leads_followup_list() {
		...
		$lead_id = udecode($this->request->getVar('xlead_id',FILTER_DEFAULT));
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
	...
	public function add_followup() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$lead_id = udecode($this->request->getPost('token',FILTER_DEFAULT));
				$next_follow_up = $this->request->getPost('next_follow_up',FILTER_DEFAULT);	
	...
	public function convert_lead() {
		...
		$request = \Config\Services::request();
		$usession = $session->get('sup_username');	
		if ($this->request->getPost('type',FILTER_DEFAULT) === 'edit_record') {
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
			$Return['csrf_hash'] = csrf_hash();	
			$id = udecode($this->request->getPost('token',FILTER_DEFAULT));
			$data = [
