...
class Settings extends BaseController {
	...
	public function system_info() {
		...
		$application_name = $this->request->getPost('application_name',FILTER_DEFAULT);
		$company_name = $this->request->getPost('company_name',FILTER_DEFAULT);
		$auth_background = $this->request->getPost('auth_background',FILTER_DEFAULT);
		$is_ssl_available = $this->request->getPost('is_ssl_available',FILTER_DEFAULT);
		if($is_ssl_available == ''){
			$is_ssl_available = 0;
		}
		$id = 1;
	...
	public function email_info() {
		...
		$email_type = $this->request->getPost('email_type',FILTER_DEFAULT);
		$email_notification = $this->request->getPost('email_notification',FILTER_DEFAULT);
	...
	public function sms_info() {
		...
		$service_plan_id = $this->request->getPost('service_plan_id',FILTER_DEFAULT);
		$bearer_token = $this->request->getPost('bearer_token',FILTER_DEFAULT);
		$sms_from = $this->request->getPost('sms_from',FILTER_DEFAULT);
		$sms_notification = $this->request->getPost('sms_notification',FILTER_DEFAULT);
	...
	public function update_currency() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			$currency_val = $this->request->getPost('currency_val',FILTER_DEFAULT);
	...
	public function currency_list() {
	 ...
	 public function currency_type_info() {
		...
		$currency_name = $this->request->getPost('currency_name',FILTER_DEFAULT);
		$currency_code = $this->request->getPost('currency_code',FILTER_DEFAULT);
		$data = [
	 ...
	 public function update_company_type() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$name = $this->request->getPost('name',FILTER_DEFAULT);
				$id = udecode($this->request->getPost('token',FILTER_DEFAULT));
				$data = [
					'category_name' => $name
				];
	...
	// update record
	 public function update_template() {
		...
		$name = $this->request->getPost('name',FILTER_DEFAULT);
		$subject = $this->request->getPost('subject',FILTER_DEFAULT);
		$status = $this->request->getPost('status',FILTER_DEFAULT);
		$message = $this->request->getPost('message',FILTER_DEFAULT);
		$id = udecode($this->request->getPost('token',FILTER_DEFAULT));
		$data = [
            'name' => $name,
			'subject' => $subject,
