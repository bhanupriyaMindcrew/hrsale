	...
	public function add_leave_type() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$name = $this->request->getPost('name',FILTER_DEFAULT);		
				$days_per_year = $this->request->getPost('days_per_year',FILTER_DEFAULT);	
				$requires_approval = $this->request->getPost('requires_approval',FILTER_DEFAULT);		
				$UsersModel = new UsersModel();
				$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
				if($user_info['user_type'] == 'staff'){
					$company_id = $user_info['company_id'];
	...
	public function add_tax_type() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$name = $this->request->getPost('name',FILTER_DEFAULT);	
				$fieldone = $this->request->getPost('fieldone',FILTER_DEFAULT);	
				$fieldtwo = $this->request->getPost('fieldtwo',FILTER_DEFAULT);			
				$UsersModel = new UsersModel();
				$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
				if($user_info['user_type'] == 'staff'){
					$company_id = $user_info['company_id'];
	...
	public function update_constants_type() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$erp_constant = udecode($this->request->getPost('erp_constant',FILTER_DEFAULT));
				$UsersModel = new UsersModel();
				$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
				if($user_info['user_type'] == 'staff'){
					$company_id = $user_info['company_id'];
