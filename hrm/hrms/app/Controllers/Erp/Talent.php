	...
	public function add_indicator() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					foreach($this->request->getPost('technical_competencies_value',FILTER_DEFAULT) as $key=>$tech_value){
						$data_ind = array(
						'company_id' => $company_id,
					...
					foreach($this->request->getPost('organizational_competencies_value',FILTER_DEFAULT) as $ikey=>$org_value){
						$data_org = array(
						'company_id' => $company_id,
						'indicator_id' => $indicator_id,
