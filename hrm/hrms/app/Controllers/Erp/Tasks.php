...
class Tasks extends BaseController {
	...
	public function add_task() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$task_name = $this->request->getPost('task_name',FILTER_DEFAULT);
				$start_date = $this->request->getPost('start_date',FILTER_DEFAULT);
				$end_date = $this->request->getPost('end_date',FILTER_DEFAULT);
				$project_id = $this->request->getPost('project_id',FILTER_DEFAULT);
				$task_hour = $this->request->getPost('task_hour',FILTER_DEFAULT);
			...
			if(!$this->validate($rules)){
				...
				if ($result == TRUE) {
					...
					if($xin_system['enable_email_notification'] == 1){
						...
						foreach($this->request->getPost('assigned_to',FILTER_DEFAULT) as $_staff_id){
	...
	public function update_task_status() {
		...
		if($this->request->getVar('xfieldid')) {
			...
			$request = \Config\Services::request();
			$usession = $session->get('sup_username');
			$task_id = $this->request->getVar('xfieldid',FILTER_DEFAULT);	
			//$task_id = $request->uri->getSegment(4);
			$task_status = $this->request->getVar('xfieldst',FILTER_DEFAULT);	
			$Return['csrf_hash'] = csrf_hash();
			$UsersModel = new UsersModel();
			$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
			if($user_info['user_type'] == 'staff'){
