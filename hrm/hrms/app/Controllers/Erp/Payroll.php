...
class Payroll extends BaseController {
	...
	public function payslip_list() {
		...
		$payment_date = $this->request->getVar('payment_date',FILTER_DEFAULT);
		$employee_id = $this->request->getVar('staff_id',FILTER_DEFAULT);
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
	...
	public function add_advance_salary() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$month_year = $this->request->getPost('month_year',FILTER_DEFAULT);
				$advance_amount = $this->request->getPost('advance_amount',FILTER_DEFAULT);
				$one_time_deduct = $this->request->getPost('one_time_deduct',FILTER_DEFAULT);
				$reason = $this->request->getPost('reason',FILTER_DEFAULT);
				// get one time value
				if($one_time_deduct==1){
					$emi_amount = $advance_amount;
				} else {
					$iemi_amount = $this->request->getPost('emi_amount',FILTER_DEFAULT);
					if($iemi_amount == ''){
						$emi_amount = 0;
					} else {
						$emi_amount = $this->request->getPost('emi_amount',FILTER_DEFAULT);
					}
				}
				...
				if($user_info['user_type'] == 'staff'){
					...
					$assigned_ids = $this->request->getPost('employee_id',FILTER_DEFAULT);
	...
	public function add_pay_monthly() {
		...
		if ($this->request->getPost('type') === 'add_monthly_payment') {
			...
			$id = udecode($this->request->getPost('token',FILTER_DEFAULT));
			$salary_month = udecode($this->request->getPost('token2',FILTER_DEFAULT));
			$MainModel = new MainModel();
			$ContractModel = new ContractModel();
			$AdvancesalaryModel = new AdvancesalaryModel();
			...
			// add info
			$payslip_comments = $this->request->getPost('payslip_comments',FILTER_DEFAULT);
			$data = [
