...
class Invoices extends BaseController {
	...
	public function client_profile_invoices_list() {
		...
		$UsersModel = new UsersModel();
		$ProjectsModel = new ProjectsModel();
		$client_id = udecode($this->request->getVar('client_id',FILTER_DEFAULT));
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
	...
	public function create_new_invoice() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$invoice_number = $this->request->getPost('invoice_number',FILTER_DEFAULT);
				$project_id = $this->request->getPost('project',FILTER_DEFAULT);
				$invoice_date = $this->request->getPost('invoice_date',FILTER_DEFAULT);
				$invoice_due_date = $this->request->getPost('invoice_due_date',FILTER_DEFAULT);
				$j=0;
				foreach($this->request->getPost('item_name',FILTER_DEFAULT) as $items){
					$item_name = $this->request->getPost('item_name',FILTER_DEFAULT);
					$iname = $item_name[$j];
					// item qty
					$qty = $this->request->getPost('qty_hrs',FILTER_DEFAULT);
					$qtyhrs = $qty[$j];
					// item price
					$unit_price = $this->request->getPost('unit_price',FILTER_DEFAULT);
					$price = $unit_price[$j];
				...
				$items_sub_total = $this->request->getPost('items_sub_total',FILTER_DEFAULT);
				$discount_type = $this->request->getPost('discount_type',FILTER_DEFAULT);
				$discount_figure = $this->request->getPost('discount_figure',FILTER_DEFAULT);
				$discount_amount = $this->request->getPost('discount_amount',FILTER_DEFAULT);
				$tax_type = $this->request->getPost('tax_type',FILTER_DEFAULT);
				$tax_rate = $this->request->getPost('tax_rate',FILTER_DEFAULT);
				$fgrand_total = $this->request->getPost('fgrand_total',FILTER_DEFAULT);
				$invoice_note = $this->request->getPost('invoice_note',FILTER_DEFAULT);
				...
				if ($result == TRUE) {
					...
					foreach($this->request->getPost('item_name',FILTER_DEFAULT) as $items){
						...
						// item sub_total
						$sub_total_item = $this->request->getPost('sub_total_item',FILTER_DEFAULT);
	...
	public function update_invoice() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$invoice_due_date = $this->request->getPost('invoice_due_date',FILTER_DEFAULT);
				$j=0;
				foreach($this->request->getPost('eitem_name',FILTER_DEFAULT) as $eitem_id=>$key_val){
					$item_name = $this->request->getPost('eitem_name',FILTER_DEFAULT);
					$iname = $item_name[$eitem_id];
					// item qty
					$qty = $this->request->getPost('eqty_hrs',FILTER_DEFAULT);
					$qtyhrs = $qty[$eitem_id];
					// item price
					$unit_price = $this->request->getPost('eunit_price',FILTER_DEFAULT);
					...
					$price = $unit_price[$eitem_id]; 
					// item sub_total
					$sub_total_item = $this->request->getPost('esub_total_item',FILTER_DEFAULT);
	...
	public function delete_invoice_items() {
		...
		if($this->request->getVar('record_id')) {
			...
			$request = \Config\Services::request();
			$usession = $session->get('sup_username');
			$record_id = udecode($this->request->getVar('record_id',FILTER_DEFAULT));
			$Return['csrf_hash'] = csrf_hash();
			$InvoiceitemsModel = new InvoiceitemsModel();
	...
	public function pay_invoice_record() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$payment_method = $this->request->getPost('payment_method',FILTER_DEFAULT);	
